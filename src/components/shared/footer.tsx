...
export function Footer() {
  ...
  const platformLinks = isDirectoryProduct ? footerLinks.platform : [...marketingPlatformLinks]
          ...
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr_1fr]">
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-7">
              <div className="flex items-center gap-3">
                <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-2xl border border-white/12 bg-white/8 p-1">
                  <img
  ...
  if (recipe.footer === 'editorial-footer') {
    return (
      <footer className="border-t border-neutral-200 bg-[linear-gradient(180deg,#ffffff_0%,#fafafa_100%)] text-neutral-950">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr_0.9fr]">
            ...
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-[0.24em] text-neutral-500">Product</h4>
        ...
        <div className="grid gap-10 md:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr_0.8fr]">
          <div>
            <Link href="/" className="flex items-center gap-3">
              <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-white p-1 shadow-sm">
                <img
                  src="/favicon.png?v=20260423"
                  alt={`${SITE_CONFIG.name} logo`}
                  width="56"
          ...
          {(['platform', 'company', 'resources', 'legal'] as const).map((section) => (
            <div key={section}>
              <h3 className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-500">{section === 'platform' ? 'Product' : section}</h3>
              <ul className="mt-5 space-y-3 text-sm text-slate-600">
                {(section === 'platform' ? platformLinks : footerLinks[section]).map((item: any) => (
